...
function App() {
  ...
  const correctId = 1440002;
  const chainIdHex = '0x' + correctId.toString(16).toUpperCase();
  ...
  const switchNetwork = async () => {
    try {
      await window.ethereum.request({
        ...
        params: [{ chainId: chainIdHex }], // Chain ID in hex format
        // params: [{ chainId: '0x3e9' }],
      });
    } catch (error) {
      toast.error((
        <div>Some error occured while switching network <br/> Please switch to XRPL EVM sidechain</div>
      ), {
        position: "top-center"
      })
      console.log(error);
